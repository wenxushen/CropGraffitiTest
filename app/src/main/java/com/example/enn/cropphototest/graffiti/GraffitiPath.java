...
public class GraffitiPath implements Undoable {
    ...
    public float[] getDxDy(int currentDegree) {
        ...
        return DrawUtil.rotatePointInGraffiti(currentDegree, mRotateDegree, mDx, mDy, mPivotX, mPivotY);
    }

    public float[] getSxSy(int currentDegree) {

        return DrawUtil.rotatePointInGraffiti(currentDegree, mRotateDegree, mSx, mSy, mPivotX, mPivotY);
    }
    ...
    public Matrix getMatrix(int currentDegree) {
        ...
        if (mPen == GraffitiView.Pen.COPY) { // 仿制，加上mCopyLocation记录的偏移
            ...
            float[] coords = DrawUtil.rotatePoint(degree, mCopy.getTouchStartX(), mCopy.getTouchStartY(), px, py);
            float[] coordsCopy = DrawUtil.rotatePoint(degree, mCopy.getCopyStartX(), mCopy.getCopyStartY(), px, py);
            if (Math.abs(degree) == 90 || Math.abs(degree) == 270) {
                coords[0] += (py - px);
                coords[1] += -(py - px);
                coordsCopy[0] += (py - px);
