...
public class CopyLocation {
    ...
    public void rotatePosition(int originalDegree, int mRotateDegree, float mOriginalPivotX, float mOriginalPivotY) {
        // 旋转仿制图标的位置
        float[] coords = DrawUtil.rotatePointInGraffiti(mRotateDegree, originalDegree, this.mX,
                this.mY, mOriginalPivotX, mOriginalPivotY);
        this.mX = coords[0];
        this.mY = coords[1];

        coords = DrawUtil.rotatePointInGraffiti(mRotateDegree, originalDegree, this.mCopyStartX,
                this.mCopyStartY, mOriginalPivotX, mOriginalPivotY);
        this.mCopyStartX = coords[0];
        this.mCopyStartY = coords[1];

        coords = DrawUtil.rotatePointInGraffiti(mRotateDegree, originalDegree, this.mTouchStartX,
