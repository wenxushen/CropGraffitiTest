...
public abstract class GraffitiSelectableItem implements Undoable {
    ...
    public void setXy(int currentRotate, float x, float y) {
        float[] xy = DrawUtil.restoreRotatePointInGraffiti(currentRotate, mGraffitiDegree, x, y, mPivotX, mPivotY);
        mX = xy[0];
        mY = xy[1];
    }

    public float[] getXy(int currentDegree) {
        return DrawUtil.rotatePointInGraffiti(currentDegree, mGraffitiDegree, mX, mY, mPivotX, mPivotY);
    }
    ...
    public boolean isInIt(int currentRotate, float x, float y, GraffitiView.Pen pen) {
        ...
        float[] rectXy = DrawUtil.rotatePoint((int) -(currentRotate - mGraffitiDegree + mItemRotate), x, y, 0, 0);
    ...
    public boolean isCanRotate(int currentRotate, float x, float y) {
        ...
        return rectXy[0] >= mRect.right && rectXy[0] <= mRect.right + ITEM_CAN_ROTATE_BOUND * DrawUtil.GRAFFITI_PIXEL_UNIT
