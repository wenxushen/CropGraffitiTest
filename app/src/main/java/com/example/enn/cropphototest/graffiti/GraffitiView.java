...
*/
            ...
            case MotionEvent.ACTION_MOVE:
                if (mTouchMode < 2) { // 单点滑动
                    ...
                    if (isPenSelectable()) {
                        if (mIsRotatingSelectedItem) {
                            float[] xy = mSelectedItem.getXy(mGraffitiRotateDegree);
                            mSelectedItem.setItemRotate(mRotateTextDiff + DrawUtil.computeAngle(
    ...
    private void setBG() {// 不用resize preview
        ...
        DrawUtil.setGraffitiPixelUnit(DensityUtil.dip2px(getContext(), 1) / mPrivateScale);

        mPaintSize = 30 * DrawUtil.GRAFFITI_PIXEL_UNIT;
...
//            }
            ...
            mPaint.setStrokeWidth(2 * DrawUtil.GRAFFITI_PIXEL_UNIT);
            canvas.drawRect(rect, mPaint);
            // rotate
            mPaint.setStyle(Paint.Style.STROKE);
            mPaint.setStrokeWidth(4 * DrawUtil.GRAFFITI_PIXEL_UNIT);
            canvas.drawLine(rect.right, rect.top + rect.height() / 2,
                    rect.right + (GraffitiSelectableItem.ITEM_CAN_ROTATE_BOUND - 16) * DrawUtil.GRAFFITI_PIXEL_UNIT, rect.top + rect.height() / 2, mPaint);
            canvas.drawCircle(rect.right + (GraffitiSelectableItem.ITEM_CAN_ROTATE_BOUND - 8) * DrawUtil.GRAFFITI_PIXEL_UNIT, rect.top + rect.height() / 2, 8 * DrawUtil.GRAFFITI_PIXEL_UNIT, mPaint);
    ...
    public final void addPath(GraffitiPath path) {
        ...
        mGraffitiListener.onAddGraffiti();
    }
    ...
    public final void removePath(GraffitiPath path) {
        ...
        draw(mBitmapCanvas, mPathStack);
        invalidate();
        mGraffitiListener.onRemoveGraffiti();
    }
